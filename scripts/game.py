...
def main():
    ...
    # Astronaut prompt and response
    # astronaut_response = openai.ChatCompletion.create(
    #     model=model_engine,
    #     max_tokens=1024,
    #     messages=[
    #         {"role": "system", "content": "Could you kindly provide a random famous astronaut's information, shared from a first-person perspective without revealing their identity? Additionally, it would be appreciated if you could offer a few multiple-choice options for astronauts, any of whom could be the subject of the previously shared information.  Don't say the correct answer."},
    #     ])

    # print(astronaut_response.choices[0]['message']['content'])

    # # Get user guess for astronaut
    # user_guess_astronaut = input("Guess the astronaut: ")
    ...
    #     max_tokens=1024,
    #     messages=[
    #         {"role": "system", "content": "Could you kindly provide a random famous astronaut's information, shared from a first-person perspective without revealing their identity? Additionally, it would be appreciated if you could offer a few multiple-choice options for astronauts, any of whom could be the subject of the previously shared information.  If the user is correct say \"YES! That is correct.\".  Otherwise say \"No, the correct answer is (name of Atronaut)"},
    #         {"role": "user", "content": astronaut_response.choices[0]['message']['content']},
    #         {"role": "user", "content": f"Is it {user_guess_astronaut}?"},
    #     ])
